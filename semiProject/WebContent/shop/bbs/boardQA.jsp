...
%>
	...
	<div id="container">
		<!--  -->
		<div id="wrapper_title">자유 게시판</div>
		<!-- 게시판 목록 시작 { -->
		<div id="bo_list" style="width: 100%">
					...
					<table>
						...
						<% } else { %>
							...
							<% if(board.getStatus() == 0) { %>
								...
								<% if (board.getNum() != board.getRef()) { %> <span style="margin-left: 20px;">└[답글]</span> <% } %>
									<%=board.getSubject() %>
										</a>
									</div>
			...
			<fieldset id="bo_sch">
				...
				<form action="<%=request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA" name="fsearch" method="post">
    		...
    		<% } else if(loginMember != null && loginMember.getStatus()==1) { %>
    		<div class="btn_confirm write_div" style="float:right;" id="btn">
    		<input type="button" id="writeBtn" accesskey="s" class="btn_submit btn" value="글쓰기">
    		</div>
    		<% } %>
		...
		<% if(startPage > blockSize) { %>
	    	<a href="<%= request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA&pageNum=1&search=<%=search%>&keyword=<%=keyword%>"><i class="fas fa-angle-double-left"></i></a>
		    <a href="<%= request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA&pageNum=<%=startPage-blockSize%>&search=<%=search%>&keyword=<%=keyword%>"><i class="fas fa-angle-left"></i></a>&nbsp;
        <% } else{ %>
        ...
        <% for (int i = startPage; i <= endPage; i++) { %>
			<% if(pageNum!=i){ %>
				<a href="<%= request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA&pageNum=<%=i%>&search=<%=search%>&keyword=<%=keyword%>"><%= i %></a>&nbsp;
			<% } else { %>
        ...
        <% if(endPage != totalPage) { %>
	    	<a href="<%= request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA&pageNum=<%=startPage+blockSize%>&search=<%=search%>&keyword=<%=keyword%>"><i class="fas fa-angle-right"></i></a>
			<a href="<%= request.getContextPath()%>/index.jsp?part=shop/bbs&content=boardQA&pageNum=<%=totalPage%>&search=<%=search%>&keyword=<%=keyword%>"><i class="fas fa-angle-double-right"></i></a>
        <% } else { %>
...
$("#writeBtn").click(function() {
	location.href="<%=request.getContextPath()%>/index.jsp?part=shop/bbs&content=board_write&category=2";
});	
</script>